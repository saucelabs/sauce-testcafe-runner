...
const { loadRunConfig, getSuite, getAbsolutePath, prepareNpmEnv } = require('sauce-testrunner-utils');
...
async function runTestCafe (createTestCafeFn, { projectPath, assetsPath, suite, metrics = []}) {
  let testCafe;
  ...
  try {
    ...
    testCafe = await createTestCafeFn('localhost', 1337, 2337);
...
async function run (runCfgPath, suiteName, createTestCafeFn) {
  const cfg = await prepareConfiguration(runCfgPath, suiteName);
  if (!cfg) {
    return false;
  }

  const testCafeResults = await runTestCafe(createTestCafeFn, cfg);
  if (!testCafeResults) {
    return false;
  }
