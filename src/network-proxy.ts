...
export function isProxyAvailable() {
  ...
  return (
    proxy && Array.isArray(proxy.split(':')) && proxy.split(':').length > 2
  );
}
...
function findNetworkServiceOnMac(): string {
  const networkInfo = shell.exec(`${networkSetup} -listnetworkserviceorder`, {
    async: false,
  }).stdout;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes('Device: en')) {
      ...
      const serviceInfo = shell.exec(`${networkSetup} -getinfo "${service}"`, {
        async: false,
      }).stdout;
      for (const l of serviceInfo.split('\n')) {
        if (
          l.includes('IP address') &&
          !l.includes('IPv6') &&
          !l.includes('none')
        ) {
          return service;
...
function setupMacProxy(proxy: ProxySettings) {
  ...
  shell.exec(
    `sudo ${networkSetup} -setwebproxy "${networkService}" ${proxyHost} ${proxyPort}`,
    { async: false },
  );
  shell.exec(
    `sudo ${networkSetup} -setsecurewebproxy "${networkService}" ${proxyHost} ${proxyPort}`,
    { async: false },
  );
}
...
function setupWinProxy(proxy: ProxySettings) {
  ...
  shell.exec(
    `${prefix} EnableLegacyAutoProxyFeatures /t REG_DWORD /d 1 /f`,
    opts,
  );
  shell.exec(
    `${prefix} ProxyServer /t REG_SZ /d "${proxyHost}:${proxyPort}" /f`,
    opts,
  );
  shell.exec(
    `${prefix} ProxyOverride /t REG_SZ /d "localhost;127.0.0.1" /f`,
    opts,
  );
...
export function setupProxy() {
  ...
  console.log(
    `Setting system proxy settings: ${proxy.proxyHost}:${proxy.proxyPort}`,
  );
